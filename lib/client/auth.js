define(['jquery', 'store', 'bluebird', 'event-emitter'], function ($, store, Promise, EventEmitter) {
  ...
  auth = {
    ...
    signUp: function singUp(username, emailAddress, password, callback) {
      var self = this,
          query;
      ...
      request('POST', '/api/user/register', query, function(err, res) {
        if (err) {
    ...
    update: function update(id,username,emailAddress , password, callback) {
      var self = this,
          query;
      ...
      request('POST', '/api/user/' + id, query, function(err, res) {
        if (err) {
          return callback(err, null);
        }
        console.log(res);
